...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.ask_id2 = self.ask_price2 = self.bid_id2 = self.bid_price2 = 0
        self.position = self.e_position = self.f_position = 0

        self.ETFaskPrices = None
        self.ETFaskVolumes = None
        self.ETFbidPrices = None
        self.ETFbidVolumes = None

        self.FUTaskPrices = None
        self.FUTaskVolumes = None
        self.FUTbidPrices = None
        self.FUTbidVolumes = None
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            self.FUTaskPrices = ask_prices
            self.FUTaskVolumes = ask_volumes
            ...
            new_bid_price = self.ETFaskPrices[0] if self.ETFaskPrices[0] < bid_prices[0] and self.ETFaskPrices[0] != 0 else 0
            new_ask_price = self.ETFbidPrices[0] if self.ETFbidPrices[0] > ask_prices[0] and self.ETFbidPrices[0] != 0 else 0
            ...
            if self.bid_id == 0 and new_bid_price != 0 and self.f_position < POSITION_LIMIT - 4 * LOT_SIZE and self.position < POSITION_LIMIT - 2 * LOT_SIZE:
                print("bid price is : ", new_bid_price, "f_position is : ", self.f_position, "e_position is : ", self.e_position, "all pos is : ", self.position)
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            ...
            if client_order_id in self.to_hedge: 
                self.send_hedge_order(next(self.order_ids), Side.ASK, MIN_BID_NEAREST_TICK, volume)
            else: # order is dynamic hedge
    ...
    def on_order_status_message(self, client_order_id: int, fill_volume: int, remaining_volume: int,
        ...
        if remaining_volume == 0:
            if client_order_id == self.bid_id:
                ...
                self.ask_id = 0
            if client_order_id == self.bid_id2:
                self.bid_id2 = 0
            elif client_order_id == self.ask_id2:
                self.ask_id2 = 0
